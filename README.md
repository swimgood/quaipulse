...
- Polls `GET https://quaiscan.io/api/v2/transactions?filter=validated&items_count=100` every 2
...
- Diffs against transaction hashes already seen; anything new is queued
- **A separate drip-feed scheduler dispenses the queue every ~220ms** (1-3 at a time depending on
  backlog), independent of poll timing — this is what actually closes the "dead air" gaps, since
  transactions no longer wait for the next poll to start appearing; they trickle out continuously
...
- The queue caps at 60 — if activity outpaces the drip-feed for an extended stretch, the oldest
  queued transactions are dropped rather than let the backlog grow into "showing you last hour's
  activity right now"
- Click the ⓘ info control to see a live debug readout: transactions received on the last poll,
  how many were actually new, current queue depth, and a countdown to the next poll
